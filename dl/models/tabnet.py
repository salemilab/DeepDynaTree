...
class TabNetNoEmbeddings(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        #return out, M_loss, res # for embeddings
        return out, M_loss
...
class Net(torch.nn.Module):
    def __init__(
        ...
        self.input_dim = len(feat_dict[args.node_feat_cols])
        self.output_dim = len(feat_dict[args.node_label_cols.split("_cat")[0]])-1
        self.n_d = n_d
        self.n_a = n_a
        self.n_steps = n_steps
        self.gamma = gamma
        ...
        self.virtual_batch_size = virtual_batch_size
        self.embedder = EmbeddingGenerator(self.input_dim, cat_dims, cat_idxs, cat_emb_dim)
        self.post_embed_dim = self.embedder.post_embed_dim
    ...
    def forward(self, x):
        ...
        x = self.embedder(x)
        x = self.tabnet(x)
        #return x[0], x[2], info   # for embeddings
        return x[0], info
